...
public:
  ...
  typedef tpc::IndexVector IndexVector;
...
public:
  ...
  // The index is badIndex() if there is not match.
  ...
  //   opt = 0: Index pairs
  std::string show(int opt =0) const;

  // Print. Same options as show.
  std::ostream& print(std::ostream& out, int iopt =0) const;
