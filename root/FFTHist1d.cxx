...
FFTHist1d::FFTHist1d(TH1* phin, int atmin, int atmax, double ftick)
: phase0(true), tmin(atmin), tmax(atmax), htime0(phin), htime(nullptr), pfft(nullptr) {
  if ( tmax <= tmin ) {
  ...
  // Frequency histogram.
  {
    ...
    hfreq = new TH1F(hname.c_str(), htitl.c_str(), nk, 0, kmax);
  ...
  // Power histogram.
  {
    ...
    htitl += htime0->GetTitle();
    hpower = new TH1F(hname.c_str(), htitl.c_str(), nk, 0, nk);
  ...
  // Phase histogram.
  {
    ...
    htitl += htime0->GetTitle();
    hphase = new TH1F(hname.c_str(), htitl.c_str(), nk, 0, nk);
  ...
  double twopi = 2.0*pi;
  int show = 0;
  pfft = new TFFTRealComplex(nt, false);
  ...
  for ( unsigned int it=0; it<nt; ++it ) {
    unsigned int ibin = htime0->GetBin(it+tmin+1);
  ...
  for ( unsigned int ik=0; ik<nk; ++ik ) {
    ...
    unsigned int ibin = hfreq->GetBin(ik+1);
    double vr;
    double vi;
...
TH1* FFTHist1d::maketime(int atmin, int atmax) {
  ...
  double twopi = 2.0*pi;
  {
    ...
    htitl += htime0->GetTitle();
    htime = new TH1F(hname.c_str(), htitl.c_str(), ntout, atmin, atmax);
    htime->SetStats(0);
    htime->SetContour(40);
    double fzmax = 100;
    htime->GetYaxis()->SetRangeUser(-fzmax, fzmax);
    htime->GetYaxis()->SetTitle(htime0->GetZaxis()->GetTitle());
  ...
  for ( unsigned int it=0; it<ntout; ++it ) {
    ...
    unsigned int ibinout = htime->GetBin(it+1);
