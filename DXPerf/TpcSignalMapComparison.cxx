...
: m_tsm1(tsm1), m_tsm2(tsm2),
  ...
  if ( m_rop != badrop ) {
    ...
    m_chend = m_chbegin + pgeohelp->ropNChannel(m_rop) - 1;
    m_nchanref = 0;
    m_nchanmat = 0;
...
ostream& TpcSignalMapComparison::print(ostream& out, string pre) const {
  out << pre << "TpcSignalMapComparison:" << endl;
  out << pre << "               ROP: " << rop() << endl;
  out << pre << "     Channel range: [" << beginChannel() << ", " << endChannel() << "]" << endl;
  out << pre << "   Reference Nchan: " << referenceChannelCount() << endl;
  out << pre << "       Match Nchan: " << matchChannelCount() << endl;
  out << pre << "  Channel fraction: " << channelFraction() << endl;
  out << pre << "      Bin fraction: " << binFraction() << endl;
  return out;
}
